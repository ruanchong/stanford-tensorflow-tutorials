...
r = tf.case([(tf.less(x, y), lambda: x + y),
...
# YOUR CODE
print(sess.run(tf.diag(diagonal=tf.range(1, 7))))
...
print(sess.run(tf.matrix_determinant(mat)))
...
# YOUR CODE
x = tf.random_normal([300], stddev=3.0)
y = tf.random_normal([300], stddev=3.0)
difference = x - y
r = tf.cond(tf.less(tf.reduce_mean(difference), 0.0),
            lambda: tf.reduce_mean(tf.square(difference)),
            lambda: tf.reduce_sum(tf.abs(difference)))
print(sess.run(r))
